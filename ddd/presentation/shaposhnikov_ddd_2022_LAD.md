...
# Вот теперь это DDD, до этого было только ООП

- Сценарий из UseCase-а был переведен в код сервиса без искажений
- Разработчики использую термины в коде из предметной области
- Код понятен аналитикам
- На митингах все говорят на едином языке
